<div id="content" class="container col-md-12">
    ...
    if (isset($_POST['btnDelete'])) {
        ...
        $res = $db->getResult();
        unlink($res[0]['image']);
        $sql_query = "DELETE FROM blog_categories WHERE id =" . $id;
        $db->sql($sql_query);
        ...
        $sql = "SELECT image FROM blogs WHERE category_id =" . $id;
        $db->sql($sql);
        $row = $db->getResult();
        unlink($row[0]['image']);
        $sql_query = "DELETE FROM blogs WHERE category_id =" . $id;
        $db->sql($sql_query);
        $delete_blog_result = $db->getResult();
        if (!empty($delete_blog_result)) {
            $delete_blog_result = 0;
        } else {
            $delete_blog_result = 1;
        }

        if ($delete_category_result == 1 && $delete_blog_result == 1) {
