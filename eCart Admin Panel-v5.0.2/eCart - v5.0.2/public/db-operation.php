...
if (isset($_GET['get_subcategories_by_category']) && $_GET['get_subcategories_by_category'] != '') {
    $category_id = $db->escapeString($fn->xss_clean($_GET['category_id']));
    if (empty($category_id)) {
        echo '<option value="">Select Categories</option>';
        return false;
    }
    $sql = "SELECT id, name FROM `subcategory` WHERE category_id = " . $category_id . " ORDER BY id ASC ";
    $db->sql($sql);
    $res = $db->getResult();

    $options = '<option value="">Select subCategories</option>';
...
if (isset($_POST['delete_variant'])) {
    ...
    $other_images = json_decode($variant_images); /*decode from json to array*/foreach ($other_images as $other_image) {
        unlink('../' . $other_image);
    }
...
if (isset($_POST['update_web_category']) && $_POST['update_web_category'] == 1) {
    ...
    if ($permissions['categories']['update'] == 1) {
        ...
        $extension = isset(($_FILES["c_image"]["name"])['extension']) ? pathinfo($_FILES["c_image"]["name"])['extension'] : "";
        $result = isset($_FILES["c_image"]) ? $fn->validate_image($_FILES["c_image"]) : "";
        if ($result) {
            echo '<p class="alert alert-danger">Image type must jpg, jpeg, gif, or png!</p>';
            return false;
            exit();
...
if (isset($_POST['boy_id']) && isset($_POST['transfer_fund'])) {
    ...
    $db->sql($sql);
    $sql = "INSERT INTO `fund_transfers` (`delivery_boy_id`,`type`,`amount`,`opening_balance`,`closing_balance`,`status`,`message`) VALUES ('" . $id . "','" . 'credit' . "','" . $amount . "','" . $balance . "','" . $bal . "','SUCCESS','" . $message . "')";
    $db->sql($sql);
...
if (isset($_POST['update_return_request']) && $_POST['update_return_request'] == 1) {
    ...
    if ($return_status == 1) {
        ...
        $status = isset($result['status']) ? json_decode($result['status']) : [];
        ...
        if ($returned == $total) {
            ...
            $db->sql($sql);
            $res = $db->getResult();
            $status_order = isset($result['status']) ? json_decode($result['status']) : [];
...
if (isset($_POST['manage_customer_wallet']) && isset($_POST['user_id'])) {
    ...
    if ($fn->add_wallet_transaction($user_id, $type, $amount, $order_id = "", $message)) {
...
if (isset($_POST['update_withdrawal_request']) && $_POST['update_withdrawal_request'] == 1) {
    ...
    if ($status == 1) {
        if ($type == 'user') {
            ...
            $fn->add_wallet_transaction($type_id, 'credit', $amount, $order_id = "", 'Balance credited on withdrawal request cancellation.');
        }
    ...
    if ($_FILES["upload_file"]["size"] > 0 && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            if ($count != 0) {
                ...
                $emapData[2] = null !== ($db->escapeString($emapData[2])) ? trim($db->escapeString($emapData[2])) : 0; // subcategory id
                ...
                $emapData[20] = null !== ($db->escapeString($emapData[20])) ? trim($db->escapeString($emapData[20])) : ''; // HSN code 
                ...
                //     echo '<p class="alert alert-danger">HSN CODE  is empty at row - ' . $count . '</div>';
                //     return false;
                // }
                if (empty($emapData[1])) {
                    echo '<p class="alert alert-danger">Category ID  is empty or invalid at row - ' . $count . '</div>';
    ...
    if ($_FILES["update_file"]["size"] > 0 && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            if ($count != 0) {
                ...
                $emapData[12] = trim($db->escapeString($emapData[12])); // tax id

                $emapData[13] = trim($db->escapeString($emapData[13])); // variants id //
                ...
                $emapData[22] = null !== ($db->escapeString($emapData[22])) ? trim($db->escapeString($emapData[22])) : ""; // HSN code


                if (empty($emapData[0]) && !is_numeric($emapData[0])) {
                    echo '<p class="alert alert-danger">Product Id  is empty or Invalid at row - ' . $count . '</div>';
                    return false;
                }
                ...
                $sql = "SELECT * FROM product_variant WHERE id=" . $emapData[13];
                $db->sql($sql);
                $result = $db->getResult();
                if (empty($result)) {
                    echo '<p class="alert alert-danger">Variant ID  is invalid at row - ' . $count . '</div>';
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count1 != 0) {
                ...
                $sql = "UPDATE product_variant SET `product_id`='" . $emapData[0] . "',`type`='" . $emapData[14] . "',`measurement`='" . $emapData[15] . "',`measurement_unit_id`='" . $emapData[16] . "',`price`='" . $emapData[17] . "',`discounted_price`='" . $emapData[18] . "',`serve_for`='" . $emapData[19] . "',`stock`='" . $emapData[20] . "',`stock_unit_id`='" . $emapData[21] . "',`hsn_code`='" . $emapData[22] . "' WHERE id=" . $emapData[13];
                $db->sql($sql);
    ...
    if ($_FILES["upload_file"]["size"] > 0 && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count != 0) {
                ...
                if (!empty($emapData[1])) {
                    $city = $fn->get_data('city', $columns = ['name'], "id=" . $emapData[1]);
                    if (empty($city)) {
...
if (isset($_POST['boy_id']) && isset($_POST['cash_collection']) && !empty($_POST['cash_collection'])) {
    ...
    $id = $db->escapeString($fn->xss_clean($_POST['boy_id']));
    $cash = $fn->get_data('delivery_boys', $columns = ['cash_received'], "id='" . $id . "'");
    if (isset($cash[0]['cash_received'])) {
        ...
        $db->sql($sql);
        $fn->add_transaction($amount, "", $id, "", "", $message, $date);
...
if (isset($_POST['send']) && ($_POST['send'] == 1)) {
    ...
    // get ticket data
    $title = $fn->get_data('tickets', '', 'id=' . $ticket_id);
    $t_id = $title[0]['id'];
