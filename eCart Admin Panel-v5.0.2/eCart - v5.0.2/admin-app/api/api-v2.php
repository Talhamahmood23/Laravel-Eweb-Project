...
// print_r($config['low-stock-limit']);
...
if (isset($_POST['add_category']) && !empty($_POST['add_category']) && $_POST['add_category'] == 1) {
    ...
    if (!empty($category_name) && !empty($category_subtitle)) {
        ...
        $upload_image = !empty($_FILES['image']["name"]) ? 'upload/images/' . $menu_image : "";
...
if ((isset($_POST['delete_category'])) && ($_POST['delete_category'] == 1)) {
    ...
    foreach ($result as $res) {
        ...
        if (!empty($res['other_images'])) {
            ...
            foreach ($other_images as $other_image) {
                isset($other_image) ? unlink($other_image) : array();
            }
        }
    ...
    if (empty($result) && $delete_category_result == 1 && $delete_subcategory_result == 1 && $delete_product_result == 1) {
...
if ((isset($_POST['add_subcategory'])) && ($_POST['add_subcategory'] == 1)) {
    ...
    if ($_FILES['upload_image']['error'] == 0) {
        ...
        $upload_image = isset($menu_image) ? 'upload/images/' . $menu_image : array();
        ...
        if ($db->sql($sql_query)) {
            ...
            $db->sql($sql);
            $res = $db->getResult();
            $res[0]['image'] = isset($res[0]['image']) ? DOMAIN_URL . $res[0]['image'] : "";
...
if ((isset($_POST['delete_subcategory'])) && ($_POST['delete_subcategory'] == 1)) {
    ...
    if (!empty($res) && $res[0]['image']) {
        isset($res[0]['image']) ? unlink('../../' . $res[0]['image']) : '';
    }

    $sql_query = "DELETE FROM `subcategory` WHERE id=" . $id;
    if (!empty($res) && $db->sql($sql_query)) {
        $response['error'] = false;
...
bonus_method:rupees/percentage
*/
if (isset($_POST['add_delivery_boy']) && !empty($_POST['add_delivery_boy']) && ($_POST['add_delivery_boy'] == 1)) {
    ...
    if (empty($_POST['bonus_method']) || empty($_POST['name']) || empty($_POST['mobile']) || empty($_POST['address']) || empty($_POST['bonus']) || empty($_POST['dob']) || empty($_POST['bank_name']) || empty($_POST['account_number']) || empty($_POST['account_name']) || empty($_POST['ifsc_code']) || empty($_POST['password']) || empty($_FILES['driving_license']) || empty($_FILES['national_identity_card'])) {
        $response['error'] = true;
        $response['message'] = "Please pass all fields!";
        print_r(json_encode($response));
        return false;
    ...
    $bonus_method = $db->escapeString(trim($fn->xss_clean($_POST['bonus_method'])));
    ...
    if ($bonus_method == 'rupees' || $bonus_method == 'percentage') {
        $sql = "INSERT INTO delivery_boys (`name`,`mobile`,`password`,`address`,`bonus`,`bonus_method`, `driving_license`, `national_identity_card`, `dob`, `bank_account_number`, `bank_name`, `account_name`, `ifsc_code`,`other_payment_information`) VALUES ('$name', '$mobile', '$password', '$address','$bonus','$bonus_method','$dr_filename', '$nic_filename', '$dob','$account_number','$bank_name','$account_name','$ifsc_code','$other_payment_info')";
        if ($db->sql($sql)) {
        ...
        $response['error'] = true;
        $response['message'] = "bonus method should be in rupees or percentage";
    }
    print_r(json_encode($response));
}
...
if (isset($_POST['update_delivery_boy']) && !empty($_POST['update_delivery_boy']) && ($_POST['update_delivery_boy'] == 1)) {
    ...
    if (empty($_POST['bonus_method']) || empty($_POST['name']) || empty($_POST['mobile']) || empty($_POST['address']) || empty($_POST['bonus']) || empty($_POST['dob']) || empty($_POST['bank_name']) || empty($_POST['account_number']) || empty($_POST['account_name']) || empty($_POST['ifsc_code'])) {
        $response['error'] = true;
        $response['message'] = "Please pass all fields!";
        print_r(json_encode($response));
        return false;
    ...
    if (!empty($res1)) {
        ...
        if ($bonus_method == 'rupees' || $bonus_method == 'percentage') {
            if (!empty($password)) {
                $sql = "Update delivery_boys set `name`='" . $name . "',`mobile`='" . $mobile . "',password='" . $password . "',`address`='" . $address . "',`bonus`='" . $bonus . "',`bonus_method`='" . $bonus_method . "',`status`='" . $status . "',`dob`='$dob',`bank_account_number`='$account_number',`bank_name`='$bank_name',`account_name`='$account_name',`ifsc_code`='$ifsc_code',`other_payment_information`='$other_payment_info' where `id`=" . $id;
            } else {
                $sql = "Update delivery_boys set `name`='" . $name . "',`mobile`='" . $mobile . "',`address`='" . $address . "',`bonus`='" . $bonus . "',`bonus_method`='" . $bonus_method . "',`status`='" . $status . "',`dob`='$dob',`bank_account_number`='$account_number',`bank_name`='$bank_name',`account_name`='$account_name',`ifsc_code`='$ifsc_code',`other_payment_information`='$other_payment_info'  where `id`=" . $id;
            }
...
if (isset($_POST['add_products']) && !empty($_POST['add_products']) && ($_POST['add_products'] == 1)) {
    ...
    $name_product = $db->escapeString($fn->xss_clean($_POST['name']));
    $tax_id = (isset($_POST['tax_id']) && $_POST['tax_id'] != '') ? $db->escapeString($fn->xss_clean($_POST['tax_id'])) : 0;
    $slug_product = $function->slugify($db->escapeString($fn->xss_clean($_POST['name'])));
    ...
    $status_product = (isset($_POST['status']) && $_POST['status'] != '') ? $db->escapeString($fn->xss_clean($_POST['status'])) : '1';
    ...
    $extension_product = pathinfo($_FILES["image"]["name"])['extension'];
    ...
    if ($image_error > 0) {
        ...
        $result_main_image = $fn->validate_image($_FILES["image"]);
    ...
    if (isset($_FILES["other_images"]) && $_FILES["other_images"]["error"] == 0) {
        for ($i = 0; $i < count((array) $_FILES["other_images"]["name"]); $i++) {
            if ($_FILES["other_images"]["error"][$i] > 0) {
                ...
                $result_other_image = $fn->validate_other_images($_FILES["other_images"]["tmp_name"][$i], $_FILES["other_images"]["type"][$i]);
                if ($result_other_image) {
                    $response['error'] = true;
    ...
    if (isset($_FILES['size_chart'])) {
        ...
        $target_path1_size_chart = '../../upload/images/';
        if ($_FILES['size_chart']['error'] == 0) {
            if (!is_dir($target_path1_size_chart)) {
                mkdir($target_path1_size_chart, 0777, true);
            }
            $extension1_size_chart = pathinfo($_FILES["size_chart"]["name"])['extension'];

            $result1_size_chart = $fn->validate_image($_FILES["size_chart"]);
            if ($result1_size_chart) {
                $response['error'] = true;
            ...
            $string1_size = '0123456789';
            $file1_size = preg_replace("/\s+/", "_", $_FILES['size_chart']['name']);
            $size_chart = $function->get_random_string($string1_size, 4) . "-" . date("Y-m-d") . "." . $extension1_size_chart;

            $upload1_size = move_uploaded_file($_FILES['size_chart']['tmp_name'], '../../upload/images/' . $size_chart);
        }
    }

    $string_product = '0123456789';
    $file_product = preg_replace("/\s+/", "_", $_FILES['image']['name']);

    $image = $function->get_random_string($string_product, 4) . "-" . date("Y-m-d") . "." . $extension_product;


    $upload_main_image = move_uploaded_file($_FILES['image']['tmp_name'], '../../upload/images/' . $image);
    ...
    if (isset($_FILES['other_images']) && ($_FILES['other_images']['size'][0] > 0)) {
        ...
        $file_data = array();
        $target_path_product = '../../upload/other_images/';
        $target_path1_product = 'upload/other_images/';
        for ($i = 0; $i < count($_FILES["other_images"]["name"]); $i++) {
            ...
            $file_data[] = $target_path1_product . '' . $filename;
            if (!move_uploaded_file($_FILES["other_images"]["tmp_name"][$i], $target_path_product . '' . $filename)) {
                $response['error'] = true;
                $response['message'] = "Other Images not uploaded!";
                print_r(json_encode($response));
                return false;
        ...
        $other_images_product = !empty($file_data) ? json_encode($file_data) : "";
    }
    $upload_image1_size = !empty($_FILES["size_chart"]) ? 'upload/images/' . $size_chart : "";
    $upload_main_image = 'upload/images/' . $image;

    $sql = "INSERT INTO products (size_chart,name,tax_id,slug,category_id,subcategory_id,image,other_images,description,shipping_delivery,indicator,manufacturer,made_in,return_status,cancelable_status, till_status,is_cod_allowed,status) VALUES('$upload_image1_size','$name_product','$tax_id','$slug_product','$category_id','$subcategory_id','$upload_main_image','$other_images_product','$description','$shipping_delivery','$indicator','$manufacturer','$made_in','$return_status','$cancelable_status','$till_status','$is_cod_allowed','$status_product')";
    $db->sql($sql);
    ...
    if ($product_result == 1) {
        $product_id = $db->escapeString($res_inner1[0]['id']);
        ...
        $hsn_code = ($_POST['hsn_code'] || !empty($_POST['hsn_code']) || $_POST['hsn_code'] != "") ? $db->escapeString($fn->xss_clean($_POST['hsn_code'])) : "";

        $measurement = isset($measurement) ? explode(",", $measurement) : '';
        $sku = isset($sku) ? explode(",", $sku) : '';
        $measurement_unit_id = isset($measurement_unit_id) ? explode(",", $measurement_unit_id) : '';
        $price = isset($price) ? explode(",", $price) : '';
        $discounted_price = isset($discounted_price) ? explode(",", $discounted_price) : '';
        $serve_for = isset($serve_for1) ? explode(",", $serve_for1) : '';

        $stock = isset($stock1) ? explode(",", $stock1) : '';
        $stock_unit_id = isset($stock_unit_id1) ? explode(",", $stock_unit_id1) : '';

        if ($_POST['type'] == 'packet') {
            for ($i = 0; $i < count($measurement); $i++) {

                $variant_images_packet = '';
                if ($_FILES["images"]["error"][$i][0] == 0) {
                    if (!empty($_FILES["images"]["name"])) {
                        for ($j = 0; $j < count((array) $_FILES["images"]["name"][$i]); $j++) {
                            if ($_FILES["images"]["error"][$i][$j] > 0) {
                                $error['images'] = " <span class='label label-danger'>Variant Images not uploaded!</span>";
                            } else {
                                $result_packet = $fn->validate_other_images($_FILES["images"]["tmp_name"][$i][$j], $_FILES["images"]["type"][$i][$j]);
                ...
                if (isset($_FILES['images']) && (!empty($_FILES['images']['name'][$i][0])) && ($_FILES['images']['size'][$i][0] > 0)) {
                    ...
                    $target_path1_packet = '../../upload/variant_images/';
                    $target_path_packet = 'upload/variant_images/';

                    if (!is_dir($target_path1_packet)) {
                        mkdir($target_path1_packet, 0777, true);
                    }
                    for ($k = 0; $k < count((array) $_FILES["images"]["name"][$i]); $k++) {
                        $filename_packet = $_FILES["images"]["name"][$i][$k];
                        $temp = explode('.', $filename_packet);
                        $filename_packet = microtime(true) . '-' . rand(100, 999) . '.' . end($temp);
                        $file_data[] = $target_path_packet . '' . $filename_packet;
                        if (!move_uploaded_file($_FILES["images"]["tmp_name"][$i][$k], $target_path1_packet . '' . $filename_packet))

                            echo "{$_FILES['images']['name'][$i][$k]} not uploaded<br/>";
                    }
                    $variant_images_packet = isset($file_data) ? json_encode($file_data) : '';
                    $variant_images_packet = isset($variant_images_packet) ? str_replace('"', "'", $variant_images_packet) : '';
                }
            ...
            for ($i = 0; $i < count($measurement); $i++) {
                    ...
                    if (!empty($_FILES["images"]["name"])) {
                        ...
                        for ($j = 0; $j < count((array) $_FILES["images"]["name"][$i]); $j++) {
                            if ($_FILES["images"]["error"][$i][$j] > 0) {
                                ...
                                $result_loose = $fn->validate_other_images($_FILES["images"]["tmp_name"][$i][$j], $_FILES["images"]["type"][$i][$j]);
                                if ($result_loose) {
                                    $error['images'] = " <span class='label label-danger'>Variant Image type must jpg, jpeg, gif, or png!</span>";
                                }
                            }
                ...
                if (isset($_FILES['images']) && (!empty($_FILES['images']['name'][$i][0])) && ($_FILES['images']['size'][$i][0] > 0)) {
                    $file_data_loose = array();
                    $target_path1_loose = '../../upload/variant_images/';
                    $target_path_loose = 'upload/variant_images/';
                    if (!is_dir($target_path1_loose)) {
                        mkdir($target_path1_loose, 0777, true);
                    }

                    for ($k = 0; $k < count((array) $_FILES["images"]["name"][$i]); $k++) {
                        $filename_loose = $_FILES["images"]["name"][$i][$k];
                        $temp_p = explode('.', $filename_loose);
                        $filename_loose = microtime(true) . '-' . rand(100, 999) . '.' . end($temp_p);
                        $file_data_loose[] = $target_path_loose . '' . $filename_loose;
                        if (!move_uploaded_file($_FILES["images"]["tmp_name"][$i][$k], $target_path1_loose . '' . $filename_loose))
                            echo "{$_FILES['images']['name'][$i][$k]} not uploaded<br/>";
                    }

                    $variant_images_loose = isset($file_data_loose) ? json_encode($file_data_loose) : "";
                    $variant_images_loose = isset($variant_images_loose) ? str_replace('"', "'", $variant_images_loose) : "";
                }
        ...
        $products = $fn->get_data('products', $columns = ['*'], '', '', '', 'ORDER BY id DESC');

        // $sql = "SELECT * from products ORDER BY id DESC LIMIT 0,1";
        // $db->sql($sql);
        // $res_inner2 = $db->getResult();
        $product = array();
        $i = 0;
        $a = $products[0]['id'];
        // $fn->get_product_id_by_variant_id($res_inner2[$i]);

        // $sql1 = "SELECT * from products WHERE id =" .  $a;
        // $db->sql($sql1);
        // $res_inner4 = $db->getResult();
        $products1 = $fn->get_data('products', $columns = ['*'], 'id =' . $a);


        foreach ($products1 as $row) {
            $variants = $fn->get_data('product_variant', $columns = ['*,(SELECT short_code FROM unit u WHERE u.id=product_variant.measurement_unit_id) as measurement_unit_name,(SELECT short_code FROM unit u WHERE u.id=product_variant.stock_unit_id) as stock_unit_name'], 'product_variant.product_id="' . $row['id'] . '" ');

            // $sql = "SELECT *,
            // (SELECT short_code FROM unit u WHERE u.id=pv.measurement_unit_id) 
            // as measurement_unit_name,
            // (SELECT short_code FROM unit u WHERE u.id=pv.stock_unit_id) 
            // as stock_unit_name
            //  FROM product_variant pv WHERE pv.product_id=" . $row['id'] . " ";
            // $db->sql($sql);
            ...
            for ($j = 0; $j < count((array) $row['other_images']); $j++) {
            ...
            if ($row['tax_id'] == 0) {
                ...
                $t_id = $row['tax_id'];
                $res_tax1 = $fn->get_data('taxes', $columns = ['*'], 'id =' . $t_id);
            ...
            for ($k = 0; $k < count((array) $variants); $k++) {
                ...
                for ($j = 0; $j < count((array) $variants[$k]['images']); $j++) {
                    $variants[$k]['images'][$j] = !empty(DOMAIN_URL . $variants[$k]['images'][$j]) ? DOMAIN_URL . $variants[$k]['images'][$j] : array();
                }
...
if (isset($_POST['update_products']) && !empty($_POST['update_products']) && ($_POST['update_products'] == 1)) {
    ...
    for ($i = 0; $i < count($measurement); $i++) {
        ...
        $vari_image = $fn->get_data('product_variant', $columns = ['id', 'images'], 'id=' . $product_variant_id[$i]);
    ...
    $db->sql($sql1);
    $res_inner5 = $db->getResult();
    $product = array();
    $i = 0;
    foreach ($res_inner5 as $row) {
...
if (isset($_POST['delete_products']) && !empty($_POST['delete_products']) && ($_POST['delete_products'] == 1)) {
    ...
    if ($total[0]['total'] == 0) {
        ...
        $db->sql($sql_query);
        $res = $db->getResult();
        isset($res['image']) ? unlink('../../' . $res[0]['image']) : '';

        $sql_query1 = "SELECT size_chart FROM products WHERE id =" . $product_id;
        $db->sql($sql_query1);
        $res1 = $db->getResult();
        isset($res1[0]['size_chart']) ? unlink('../../' . $res1[0]['size_chart']) : '';
...
if (isset($_POST['get_products']) && !empty($_POST['get_products']) && ($_POST['get_products'] == 1)) {
    ...
    foreach ($res as $row) {
        ...
        for ($k = 0; $k < count($variants); $k++) {
            // $variant_images = str_replace("'", '"', $variants[$k]['images']);
            $variant_images = isset($variants[$k]['images']) ? str_replace("'", '"', $variants[$k]['images']) : '';
            ...
            $variants[$k]['sku'] = (!empty($variants[$k]['sku'])) ? $variants[$k]['sku'] : "";
            $variants[$k]['hsn_code'] = (!empty($variants[$k]['hsn_code'])) ? $variants[$k]['hsn_code'] : "";
...
if (isset($_POST['get_orders']) && !empty($_POST['get_orders'])) {
    ...
    foreach ($res as $row) {
        ...
        if ($res[$i]['delivery_boy_id'] != 0 && $res[$i]['delivery_boy_id'] != "") {
            $d_name = $fn->get_data('delivery_boys', $columns = ['name'], 'id=' . $res[$i]['delivery_boy_id']);
    ...
    if (!empty($res)) {
        ...
        if ($postStatus == 'cancelled' || $postStatus == 'returned') {
            ...
            if (strtolower($res[0]['payment_method']) != 'cod') {
                ...
                $wallet_txn_id = $fn->add_wallet_transaction($id, 'credit', $total, $user_id, 'Balance credited against item cancellation..');
            } else {
        ...
        if ($postStatus == 'delivered') {
            ...
            if ($res_boy[0]['delivery_boy_id'] != 0) {
                ...
                if ($reward > 0) {
                    ...
                    $db->sql($sql);
                    $comission = $fn->add_delivery_boy_commission($delivery_boy_id, $reward, 'credit', 'Order Delivery Commission.');
                    ...
                    if ($res_count[0]['total'] == 0) {
                        if ($res_user[0]['friends_code'] != '') {
                            ...
                            if (!empty($friend_user))
                                $fn->add_wallet_transaction($id, 'credit', floor($bonus_amount), $friend_user[0]['id'], 'Refer & Earn Bonus on first order by ' . ucwords($res_data[0]['name']));
        ...
        if ($postStatus == 'cancelled') {
            ...
            foreach ($result as $item) {
                if ($item['active_status'] != 'cancelled') {
                    ...
                    $db->update('order_items', 'id=' . $item['id'], $item_data);
                }
            }
        }
        ...
        if ($postStatus == 'received') {
            ...
            /* get order data */
            $user_id1 = $fn->get_data('orders', $columns = ['user_id', 'total', 'delivery_charge', 'discount', 'final_total', 'payment_method', 'address', 'otp'], 'id=' . $id);

            /* get user data */
            $user_email = $fn->get_data('users', $columns = ['email', 'name'], 'id=' . $user_id1[0]['user_id']);
        ...
        if ($db->update('orders', 'id=' . $id, $final_status)) {
            $response['error'] = false;
            if ($postStatus == 'cancelled') {
            ...
            if ($postStatus != 'received') {
                $user_data = $fn->get_data('users', $columns = ['name', 'email', 'mobile', 'country_code'], 'id=' . $user_id);
        ...
        if ($postStatus == 'cancelled') {
            ...
            if ($cancelled == $total) {
                if (strtolower($res_order[0]['payment_method']) != 'cod') {
                    ...
                    $fn->update_wallet_balance($new_balance, $user_id);
                    $wallet_txn_id = $fn->add_wallet_transaction($user_id, 'credit', $total_amount, $order_id, 'Balance credited against item cancellation...');
                } else {
                    if ($res_order[0]['wallet_balance'] != 0) {
                        ...
                        $fn->update_wallet_balance($new_balance, $user_id);
                        $wallet_txn_id = $fn->add_wallet_transaction($user_id, 'credit', $res_order[0]['wallet_balance'], $order_id, 'Balance credited against item cancellation!!');
                    }
                }
                ...
                $db->update('orders', 'id=' . $order_id, $data_order);
            }
...
if (isset($_POST['get_all_data']) && !empty($_POST['get_all_data'])) {
    ...
    foreach ($result as $row) {
        ...
        $product_ids = isset($row['product_ids']) ? explode(',', $row['product_ids']) : "";
        ...
        $product_ids = isset($product_ids) ? implode(',', $product_ids) : "";
...
product_id:1
image:0    // {index of other image array}
*/
...
if (isset($_POST['upload_bank_transfers_attachment']) && $_POST['upload_bank_transfers_attachment'] == 1) {
    ...
    */
    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
        for ($i = 0; $i < count((array) $_FILES["image"]["name"]); $i++) {
    ...
    if (isset($_FILES['image']) && ($_FILES['image']['size'][0] > 0)) {
        $file_data = array();
        $target_path_img = '../upload/attachments/';
        if (!is_dir($target_path_img)) {
            mkdir($target_path_img, 0777, true);
        }
        $target_path1_img = 'upload/attachments/';
        for ($i = 0; $i < count($_FILES["image"]["name"]); $i++) {
            $filename = $_FILES["image"]["name"][$i];
            $temp = explode('.', $filename);
            $filename = microtime(true) . '-' . rand(100, 999) . '.' . end($temp);
            $file_data[] = $target_path1_img . '' . $filename;
            if (!move_uploaded_file($_FILES["image"]["tmp_name"][$i], $target_path_img . '' . $filename)) {
                $response['error'] = true;
                $response['message'] = "Images not uploaded!";
                print_r(json_encode($response));
                return false;
    ...
    foreach ($res as $row) {
        ...
        $status = $res[$i]['status'];
        $item1 = array('reset', $status);
        $item2 = array('end', $status);
...
if (isset($_POST['delete_bank_transfers_attachment']) && $_POST['delete_bank_transfers_attachment'] == 1) {
    ...
    */
    ...
    $db->sql($sql);
    $image = $db->getResult();
    isset($image[0]['attachment']) ? unlink('../' . $image[0]['attachment']) : '';
    ...
    */
    ...
    foreach ($res as $row) {
        ...
        for ($j = 0; $j < count($res[$i]['items']); $j++) {
            ...
            $res[$i]['items'][$j]['till_status'] = !empty($res[$i]['items'][$j]['till_status']) ? $res[$i]['items'][$j]['till_status'] : "";
            $res[$i]['items'][$j]['review_status'] = (!empty($res[$i]['items'][$j]['review_status']) && ($res[$i]['items'][$j]['review_status'] == 1)) ? $res[$i]['items'][$j]['review_status'] == TRUE : FALSE;
            $sql = "SELECT id from return_requests where product_variant_id = " . $res[$i]['items'][$j]['variant_id'] . " OR user_id = " . $res[$i]['user_id'];
