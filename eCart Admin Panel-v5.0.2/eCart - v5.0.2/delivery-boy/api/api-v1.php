...
if (isset($_POST['login'])) {
    ...
    */
    ...
    if ($num == 1) {
        ...
        $mobileno = isset($res[0]['mobile']) ? $res[0]['mobile'] : "";
        $pass = isset($res[0]['password']) ? $res[0]['password'] : "";

        if ($mobileno != $mobile) {
            $response['error'] = true;
            $response['message'] = "Phone Number is not registered!";
        }
        if ($mobileno != $mobile && $pass != $password) {
            $response['error'] = true;
    ...
    if (!empty($res)) {
        ...
        if ($postStatus == 'cancelled' || $postStatus == 'returned') {
            ...
            if ($res[0]['payment_method'] != 'cod' && $res[0]['payment_method'] != 'COD') {
                $user_id = $res[0]['user_id'];
                $promo_dis = isset($res[0]['promo_discount']) ? $res[0]['promo_discount'] : 0;
                $total = $res[0]['total'] + $res[0]['delivery_charge'] + $res[0]['tax_amount'] - $promo_dis;
        ...
        if ($postStatus == 'delivered') {
            $res_boy = $fn->get_data('orders', $columns = ['delivery_boy_id,final_total,total,payment_method,delivery_charge'], 'id= ' . $id);
            $delivery_boy_name = $fn->get_data('delivery_boys', $columns = ['name'], 'id= ' . $res_boy[0]['delivery_boy_id']);
            $currency = $fn->get_data('settings', $columns = ['value'], 'variable= "currency" ');

            if ($res_boy[0]['delivery_boy_id'] != 0) {
                $res_bonus = $fn->get_data('delivery_boys', $columns = ['bonus,name,bonus_method'], 'id= ' . $res_boy[0]['delivery_boy_id']);
                ...
                if (strtolower($res_boy[0]['payment_method']) == 'cod') {
                    ...
                    $db->sql($sql);
                    $fn->add_transaction($cash_received, $id, $res_boy[0]['delivery_boy_id'], 'delivery_boy_cash', 'Delivery boy collected COD');
                }
                ...
                $sm = 'Order Delivery Boy Commission.';
                $comission = $fn->add_delivery_boy_commission($delivery_boy_id, $reward, 'credit', $sm, 'succeess');
...
if (isset($_POST['delivery_boy_forgot_password']) && isset($_POST['mobile'])) {
    ...
    */
    ...
    if (!empty($res_mobile) && $res_mobile[0]['mobile'] == $mobile) {
        ...
        if ($fn->is_user_or_dboy_exists($type, $type_id)) {
            ...
            if ($balance >= $amount) {
                ...
                if ($fn->debit_balance($type, $type_id, $new_balance)) {
                    ...
                    if ($type == 'user') {
                        $fn->add_wallet_transaction($type_id, 'debit', $amount, $order_id = "", 'Balance debited against withdrawal request.');
                    }
                    $new_balance = 0;
...
*/
    ...
    if (!empty($type) && !empty($type_id)) {
        ...
        if ($data_type == "withdrawal_requests") {
            ...
            $response['error'] = false;
            $response['total'] = $total[0]['total'];
            $response['data'] = array_values($total);
        }
    } else {
        $response['error'] = true;
        $response['message'] = 'Please pass all the fields!';
