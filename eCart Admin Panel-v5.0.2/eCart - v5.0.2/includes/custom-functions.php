...
class custom_functions
{
    ...
    public function update_wallet_balance($balance, $id)
    {
        ...
        if ($this->db->update('users', 'id=' . $id, $data))
            return true;
        else
            return false;
    }

    public function add_wallet_transaction($id, $type, $amount, $order_id = "", $message = 'Used against Order Placement', $status = 1)
    {
        $data = array(
            'order_id' => $order_id,
            'user_id' => $id,
    ...
    public function validate_promo_code($user_id, $promo_code, $total)
    {
        ...
        // $res_user = isset($res_user) ? $res_user : 0;
        if (empty($res_user)) {
            $response['error'] = true;
    ...
    public function add_delivery_boy_commission($id, $amount, $type, $message, $status = "SUCCESS")
    {
    ...
    public function get_data($table, $columns = [], $where = '', $join_table = '', $group_by = '', $order_by = '', $limit = '')
    {
        $sql = "select ";
        if (!empty($columns)) {
    ...
    public function delete_other_images($pid, $i)
    {
        ...
        $other_images = isset($other_images) ? json_decode($other_images) : ''; /*decode from json to array*/

        isset($other_images[$i]) ? unlink("../../" . $other_images[$i]) : ""; /*remove the image from the folder*/

        unset($other_images[$i]); /*remove image from the array*/
        $other_images = isset($other_images) ? json_encode(array_values($other_images)) : ''; /*convert back to JSON */
    ...
    public function delete_variant_images($vid, $i)
    {
        ...
        $variant_images = isset($other_images) ? str_replace("'", '"', $other_images) : '';
    ...
    public function get_delivery_charge($area_id, $total = 0)
    {
        ...
        if ($config['area-wise-delivery-charge'] == 0) {
            $area = $this->get_data('area', ['delivery_charges', 'minimum_free_delivery_order_amount'], 'id=' . $area_id);
    ...
    public function get_cart_data($fields = '', $where = '', $join_table = '', $sort = 'p.id', $order = 'DESC')
    {
        ...
        for ($j = 0; $j < count($res); $j++) {
            ...
            $variant_images = (isset($res[$j]['images']) && !empty($res[$j]['images'])) ? str_replace("'", '"', $res[$j]['images']) : "";
    ...
    public function add_transaction($amount, $order_id = "", $id = "", $txn_id = "", $type = '', $message = '', $date = '', $status = 1)
    {
