...
class functions
{
    ...
    {
        $slug2 = isset($text) ?  strtolower($text) : "";

        // replace non letter or digits by -
        $text = isset($text) ? preg_replace('~[^\pL\d]+~u', '-', $text) : "";

        // transliterate
        //   $text = iconv('utf-8', 'us-ascii//TRANSLIT', $text);
        $text = isset($text) ? iconv('utf-8', 'UTF-8//TRANSLIT', $text) : "";

        // remove unwanted characters
        $text = isset($text) ? preg_replace('~[^-\w]+~', '', $text) : "";
        ...
        // lowercase
        $slug = isset($text) ? strtolower($text) : "";
