...
for ($i = 0; $i < count($res_categories); $i++) {
    // $slug = $function->slugify($db->escapeString($fn->xss_clean($res_categories[$i]['name'])));
...
if (!empty($res_slider_image)) {
    ...
    foreach ($res_slider_image as $row) {
        ...
        $temp['image2'] = !empty($row['image2']) ?  DOMAIN_URL . $row['image2'] : array();
        $slider_images[] = $temp;
    }
}
...
foreach ($result as $row) {
    ...
    foreach ($res_section_images as $offer_row) {
        $name = "";
        $slug = "";
        if ($offer_row['offer_type'] == 'category') {

            $sql = 'select `name` from category where id = ' . $offer_row['offer_type_id'] . ' order by id desc';
            $db->sql($sql);
            $result1 = $db->getResult();
        ...
        if ($offer_row['offer_type'] == 'products') {

            $sql = 'select `name`,`slug` from products where id = ' . $offer_row['offer_type_id'] . ' order by id desc';
            $db->sql($sql);
            $result1 = $db->getResult();
            $name = (!empty($result1[0]['name'])) ? $result1[0]['name'] : "";
        ...
        $section_temp['offer_type'] = $offer_row['offer_type'];
        $section_temp['name'] = $name;
        $section_temp['slug'] = $slug;
        $section_temp['offer_type_id'] = $offer_row['offer_type_id'];
...
foreach ($res_offer_images as $row1) {
    $name = "";
    $slug = "";
    if ($row1['offer_type'] == 'category') {
        $sql = 'select `name` from category where id = ' . $row1['offer_type_id'] . ' order by id desc';
        $db->sql($sql);
        $result1 = $db->getResult();
    ...
    if ($row1['offer_type'] == 'products') {
        $sql = 'select `name`,`slug` from products where id = ' . $row1['offer_type_id'] . ' order by id desc';
        $db->sql($sql);
        $result1 = $db->getResult();
        $name = (!empty($result1[0]['name'])) ? $result1[0]['name'] : "";
        $slug = (!empty($result1[0]['slug'])) ? $result1[0]['slug'] : "";
    ...
    $temp['offer_type'] = $row1['offer_type'];
    $temp['offer_type_id'] = $row1['offer_type_id'];
    $temp['name'] = $name;
    $temp['slug'] = $slug;
    $temp['offer_image_url'] = !empty($row1['offer_image_url']) ? $row1['offer_image_url'] : "";
...
foreach ($res_offer_images as $row2) {
    ...
    $name = "";
    $slug = "";
    if ($row2['offer_type'] == 'category') {

        $sql = 'select `name` from category where id = ' . $row2['offer_type_id'] . ' order by id desc';
        $db->sql($sql);
        $result1 = $db->getResult();

        $name = (!empty($result1[0]['name'])) ? $result1[0]['name'] : "";
        $slug =
            $function->slugify($db->escapeString($fn->xss_clean($name)));
    }
    if ($row2['offer_type'] == 'products') {
        $sql = 'select `name`,`slug` from products where id = ' . $row2['offer_type_id'] . ' order by id desc';
        $db->sql($sql);
        $result1 = $db->getResult();
        $name = (!empty($result1[0]['name'])) ? $result1[0]['name'] : "";
        $slug = (!empty($result1[0]['slug'])) ? $result1[0]['slug'] : "";
    ...
    $cate_temp['offer_type'] = $row2['offer_type'];
    $cate_temp['offer_type_id'] = $row2['offer_type_id'];
    $cate_temp['name'] = $name;
    $cate_temp['slug'] = $slug;
    $cate_temp['offer_image_url'] = !empty($row2['offer_image_url']) ? $row2['offer_image_url'] : "";
...
foreach ($res_seller_images as $row3) {
    $name = "";
    $slug = "";
    if ($row3['offer_type'] == 'category') {
        $sql = 'select `name` from category where id = ' . $row3['offer_type_id'] . ' order by id desc';
        $db->sql($sql);
        $result1 = $db->getResult();
        $name = (!empty($result1[0]['name'])) ? $result1[0]['name'] : "";
        $slug = $function->slugify($db->escapeString($fn->xss_clean($name)));
    }
    if ($row3['offer_type'] == 'products') {
        $sql = 'select `name`,`slug` from products where id = ' . $row3['offer_type_id'] . ' order by id desc';
        $db->sql($sql);
        $result1 = $db->getResult();
        $name = (!empty($result1[0]['name'])) ? $result1[0]['name'] : "";
        $slug = (!empty($result1[0]['slug'])) ? $result1[0]['slug'] : "";
    ...
    $slider_temp['offer_type'] = $row3['offer_type'];
    $slider_temp['offer_type_id'] = $row3['offer_type_id'];
    $slider_temp['name'] = $name;
    $slider_temp['slug'] = $slug;
    $slider_temp['offer_image_url'] = !empty($row3['offer_image_url']) ? $row3['offer_image_url'] : "";
...
foreach ($res_flash_sales_images as $flash_sale_row) {
    $name = "";
    $slug = "";
    if ($flash_sale_row['offer_type'] == 'category') {
        $sql = 'select `name` from category where id = ' . $flash_sale_row['offer_type_id'] . ' order by id desc';
        $db->sql($sql);
        $result1 = $db->getResult();
        $name = (!empty($result1[0]['name'])) ? $result1[0]['name'] : "";
        $slug = $function->slugify($db->escapeString($fn->xss_clean($name)));
    }
    if ($flash_sale_row['offer_type'] == 'products') {
        $sql = 'select `name`,`slug` from products where id = ' . $flash_sale_row['offer_type_id'] . ' order by id desc';
        $db->sql($sql);
        $result1 = $db->getResult();
        $name = (!empty($result1[0]['name'])) ? $result1[0]['name'] : "";
        $slug = (!empty($result1[0]['slug'])) ? $result1[0]['slug'] : "";
    ...
    $flash_sales_temps['offer_type'] = $flash_sale_row['offer_type'];
    $flash_sales_temps['offer_type_id'] = $flash_sale_row['offer_type_id'];
    $flash_sales_temps['name'] = $name;
    $flash_sales_temps['slug'] = $slug;
    $flash_sales_temps['offer_image_url'] = !empty($flash_sale_row['offer_image_url']) ? $flash_sale_row['offer_image_url'] : "";
