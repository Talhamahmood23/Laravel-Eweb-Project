...
if (isset($_GET['table']) && $_GET['table'] == 'orders') {
    ...
    foreach ($res as $row) {
        ...
        $items1 = $temp;
        $temp = '';
        $status = isset($row['items'][0]['order_status']) ? json_decode($row['items'][0]['order_status']) : "";
...
if (isset($_GET['table']) && $_GET['table'] == 'products') {
    ...
    if (isset($_GET['subcategory_id']) && $_GET['subcategory_id'] != '') {
        if (isset($_GET['category_id']) and $_GET['category_id'] != '') {
            $sub_id = $db->escapeString($fn->xss_clean($_GET['subcategory_id']));
        }
        if (isset($_GET['category_id']) and $_GET['category_id'] != '') {
            $where .= ' and p.`subcategory_id`=' . $sub_id;
        } else {
            $where = ' where p.`subcategory_id`=' . $sub_id;
        }
    }
    ...
    fp.price as sales_price,
    fp.discounted_price as sales_discounted_price,
    fp.status as sales_status,
    fp.start_date,fp.end_date,p.name,p.status,p.tax_id, p.ratings,p.number_of_ratings,p.image, p.indicator, p.manufacturer, p.made_in, p.return_status, p.cancelable_status, p.till_status,p.description,
     pv.id as product_variant_id, pv.hsn_code,pv.price, pv.discounted_price, pv.measurement, pv.serve_for, pv.stock,pv.stock_unit_id, u.short_code 
            FROM `products` p
            $join 
    ...
    $currency = $fn->get_settings('currency', false); foreach ($res as $row) {
...
if (isset($_GET['table']) && $_GET['table'] == 'product_variants') {
    ...
    $db->sql($sql);
    $res = $db->getResult(); foreach ($res as $row) {
        if ($row['indicator'] == 0) {
            $indicator = "<span class='label label-info'>None</span>";
        ...
        if (!empty($row['stock_unit_id'])) {
            $stock_unit = $fn->get_data('unit', $columns = ['short_code as stock_unit'], 'id=' . $row['stock_unit_id']);
            $tempRow['stock'] = $row['stock'] . ' ' . $stock_unit[0]['stock_unit'];
        }
...
if (isset($_GET['table']) && $_GET['table'] == 'offers') {
    ...
    $rows = array();
    $tempRow = array();
    $images = DOMAIN_URL . 'images/video_images.png'; foreach ($res as $row) {
        if ($row['status'] == 1) {
        ...
        $tempRow['id'] = $row['id'];
        $tempRow['type'] = $row['type'];
        $tempRow['offer_type'] = $row['offer_type'];
        $tempRow['offer_type_id'] = $row['offer_type_id'];
        ...
        $tempRow['offer_image_url'] = $row['offer_image_url'];
        $tempRow['status'] = ($row['status'] == 1) ? '<label class="label label-success">Active</label>' : '<label class="label label-danger">Deactive</label>';
...
if (isset($_GET['table']) && $_GET['table'] == 'delivery-boys') {
    ...
    $rows = array();
    $tempRow = array();
    $path = 'upload/delivery-boy/'; foreach ($res as $row) {
...
if (isset($_GET['table']) && $_GET['table'] == 'taxes') {
    ...
    $bulkData = array();
    $bulkData['total'] = $total;
    $rows = $tempRow = array(); foreach ($res as $row) {
