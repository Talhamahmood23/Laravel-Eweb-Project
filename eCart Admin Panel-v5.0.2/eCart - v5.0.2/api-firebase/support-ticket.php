...
if ((isset($_POST['add_ticket'])) && ($_POST['add_ticket'] == 1)) {
    ...
    if ($db->sql($sql)) {
        ...
        $res[0]['image'] = isset($res[0]['image']) ? json_decode($res[0]['image'], 1) : "";
...
if ((isset($_POST['edit_ticket'])) && ($_POST['edit_ticket'] == 1)) {
    ...
    */
    ...
    if ($db->sql($sql)) {
        $res = $fn->get_data('tickets', $columns = ['tickets.*', 'ti_t.type'], '', 'LEFT JOIN ticket_type ti_t ON ti_t.id = tickets.type_id', '', 'ORDER BY tickets.id DESC', 'LIMIT 0,1');
        $ticket_id = $res[0]['id'];
...
if ((isset($_POST['send_message'])) && ($_POST['send_message'] == 1)) {
    ...
    */
    ...
    $ticket = $fn->get_data('tickets', '', 'id=' . $ticket_id);
    if (!empty($ticket)) {
