...
if ((isset($_POST['add-flash-sales'])) && ($_POST['add-flash-sales'] == 1)) {
    ...
    $res = $fn->get_data('flash_sales', $columns = ['id'], '',  '', '', 'ORDER BY id DESC');
...
if ((isset($_POST['edit-flash-sales'])) && ($_POST['edit-flash-sales'] == 1)) {
    ...
    $res = $fn->get_data('flash_sales', $columns = ['slug'], 'id != ' . $id);
    $i = 1;
    foreach ($res as $row) {
        $slug = $slug . '-' . $i;
        $i++;
...
if (isset($_POST['get-all-flash-sales']) && ($_POST['get-all-flash-sales'] == 1)) {
    ...
    */
    ...
    $result = $fn->get_data('flash_sales', '', 'status=1 ' . $where,  '', '', 'ORDER BY id ASC', 'LIMIT ' . $offset . ',' . $limit);
