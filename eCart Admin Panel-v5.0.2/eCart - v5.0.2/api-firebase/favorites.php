...
if ((isset($_POST['get_favorites'])) && ($_POST['get_favorites'] == 1)) {
    ...
    */
    ...
    if (!empty($user_id)) {
        ...
        $res = $fn->get_data('favorites', $columns = ['favorites.id AS favourite_id,favorites.user_id,favorites.product_id as id,p.row_order,p.name,p.slug,p.category_id,p.total_allowed_quantity ,p.is_cod_allowed,p.tax_id,(SELECT name from category c where c.id=p.category_id) as category_name,p.subcategory_id,p.indicator,p.manufacturer,p.made_in,p.return_status,p.cancelable_status,p.till_status,p.image,p.other_images,p.size_chart,p.description,p.shipping_delivery,p.status,p.date_added,p.ratings,p.number_of_ratings,pr.review,pr.rate'], 'favorites.user_id=' . $user_id,  'LEFT JOIN products p ON favorites.product_id=p.id left join product_reviews pr on p.id = pr.product_id', 'GROUP BY p.id', 'ORDER BY favorites.date_created DESC', 'LIMIT ' . $offset . ',' . $limit);
        ...
        if (!empty($res)) {
            foreach ($res as $row) {
                if ($fn->is_product_available($row['id'])) {
                    $result = $fn->get_data('favorites', $columns = ['id'], 'product_id = "' . $row['id'] . '" AND user_id = "'  . $user_id . '" ');
                    ...
                    if ($row['tax_id'] == 0) {
                        ...
                        $t_id = $row['tax_id'];
                        $res_tax1 = $fn->get_data('taxes', '', 'id = ' . $t_id);
                    ...
                    for ($k = 0; $k < count($variants); $k++) {
                        ...
                        $res = $fn->get_data('cart', $columns = ['qty as cart_count'], ' product_variant_id = "' . $variants[$k]['id'] . '" AND user_id = "'  . $user_id . '" ');
                        ...
                        $result = $fn->get_data('flash_sales_products', $columns = ['*,fs.title as flash_sales_name'], ' flash_sales_products.product_variant_id = "' . $variants[$k]['id'] . '" AND flash_sales_products.product_id = "'  . $variants[$k]['product_id'] . '" ', 'LEFT JOIN flash_sales fs ON fs.id=flash_sales_products.flash_sales_id');
