...
if (isset($_POST['place_order']) && isset($_POST['user_id']) && !empty($_POST['product_variant_id'])) {
    ...
    */
    ...
    if ($pickup == 1 && $local_pickup == 1) {
        ...
        if ($settings['area-wise-delivery-charge'] == 0) {
            ...
            if (isset($_POST['area_id']) && !empty($_POST['area_id'])) {
                ...
                $area = $function->get_data('area', ['delivery_charges', 'minimum_free_delivery_order_amount'], 'id=' . $area_id);
    ...
    if ($wallet_used == 'true') {
        ...
        $wallet_txn_id = $function->add_wallet_transaction($user_id, 'debit', $wallet_balance, $order_id, 'Used against Order Placement');
    }
    for ($i = 0; $i < count($item_details); $i++) {
        ...
        $hsn_code = $item_details[$i]['hsn_code'];
        ...
        $res_product = $function->get_data('products', $columns = ['name'], 'id=' . $product_id);
        $res_variant = $function->get_data('product_variant', $columns = ['measurement', 'measurement_unit_id'], 'id=' . $product_variant_id);
        $res_unit = $function->get_data('unit', $columns = ['short_code'], 'id=' . $res_variant[0]['measurement_unit_id']);
        ...
        $sql = "INSERT INTO `order_items`(`user_id`, `order_id`, `product_variant_id`, `quantity`, `price`, `discounted_price`,`tax_amount`,`tax_percentage`, `discount`, `sub_total`, `status`, `active_status`,`product_name`,`variant_name`,`product_id`,`hsn_code`) VALUES ('$user_id','$neworder_id','$product_variant_id','$quantity','$order_price','$discounteds_price','$tax_amount', $tax_percentage ,'$discount','$order_sub_total','$order_item_status','$active_status','$product_name','$variant_name','$product_id','$hsn_code')";
        $db->sql($sql);
        $res = $db->getResult();
...
if (isset($_POST['update_order_item_status']) && isset($_POST['order_item_id'])) {
    ...
    $postStatuses = $db->escapeString($function->xss_clean($_POST['status']));

    $store_pickup = $function->is_lockup($order_id);


    $sql = "SELECT COUNT(id) as cancelled FROM `order_items` WHERE id=" . $order_item_id . " && status LIKE '%$postStatuses%'";
    $db->sql($sql);
    $res_cancelled = $db->getResult();
    if ($res_cancelled[0]['cancelled'] == 'awaiting_payment' && ($postStatuses == 'returned' || $postStatuses == 'delivered' || $postStatuses == 'shipped' || $postStatuses == 'processed' || $postStatuses == 'ready_to_pickup')) {
        $response['error'] = true;
        $response['message'] = "Order item can not $postStatuses. Because it is on awaiting status.";
        print_r(json_encode($response));
        return false;
    }
    if ($res_cancelled[0]['cancelled'] > 0) {
        $response['error'] = true;
        $response['message'] = 'Could not update order status. Item is already ' . ucwords($postStatuses) . '!';
        print_r(json_encode($response));
        return false;
    }
    ...
    if (!empty($result)) {
        $status = isset($result[0]['status']) ? json_decode($result[0]['status']) : "";
        if ($postStatuses == 'cancelled') {
            ...
            $status[] = array($postStatuses, date("d-m-Y h:i:sa"));
            $currentStatus = $postStatuses;
            ...
            $sql = "select o.*,obt.status as attachment_status from orders o LEFT JOIN order_bank_transfers obt ON o.id = obt.order_id where o.id=" . $order_id;
            $db->sql($sql);
            ...
            if ($cancelled == $total) {
                if (strtolower($res_order[0]['payment_method']) != 'cod') {
                    ...
                    /* update user's wallet */
                    if ($res_order[0]['payment_method'] == "bank transfer") {
                        if ($res_current_status[0]['attachment_status'] == '1') {
                            // if receipt approved then only refund
                            $user_id = $res_order[0]['user_id'];
                            $total_amount = $res_order[0]['total'] + $res_order[0]['delivery_charge'] + $res_order[0]['tax_amount'];
                            $user_wallet_balance = $function->get_wallet_balance($user_id);
                            $new_balance = $user_wallet_balance + $total_amount;
                            $function->update_wallet_balance($new_balance, $user_id);
                            $wallet_txn_id = $function->add_wallet_transaction($user_id, 'credit', $total_amount, $order_id, 'Balance credited against item cancellation...');
                        }
                    } else {
                        // id any other other payment gateway then refund
                        $user_id = $res_order[0]['user_id'];
                        $total_amount = $res_order[0]['total'] + $res_order[0]['delivery_charge'] + $res_order[0]['tax_amount'];
                        $user_wallet_balance = $function->get_wallet_balance($user_id);
                        $new_balance = $user_wallet_balance + $total_amount;
                    ...
                    if ($res_order[0]['wallet_balance'] != 0) {
                        ...
                        if ($res[0]['payment_method'] == "bank transfer") {
                            if ($res_current_status[0]['attachment_status'] == '1') {
                                ...
                                $function->update_wallet_balance($new_balance, $user_id);
                                $wallet_txn_id = $function->add_wallet_transaction($user_id, 'credit', $res_order[0]['wallet_balance'], $order_id, 'Balance credited against item cancellation!!');
                            }
                        } else {
                            // id any other other payment gateway then refund
                            $user_id = $res_order[0]['user_id'];
...
if (isset($_POST['update_order_status']) && isset($_POST['id'])) {
    ...
    if (!empty($res)) {
            ...
            if (strtolower($res[0]['payment_method']) != 'cod') {
                ...
                if ($res[0]['payment_method'] == "bank transfer") {
                    if ($res_current_status[0]['attachment_status'] == '1') {
                        ...
                        $wallet_txn_id = $function->add_wallet_transaction($id, 'credit', $total, $user_id, 'Balance credited against item cancellation..');
                    }
                } else {
                    // id any other other payment gateway then refund
                    $user_id = $res[0]['user_id'];
        ...
        if ($postStatus == 'delivered') {
            $res_boy = $function->get_data('orders', $columns = ['delivery_boy_id,final_total,total,payment_method,delivery_charge'], 'id= ' . $id);
            $currency = $function->get_data('settings', $columns = ['value'], 'variable= "currency" ');

            if ($res_boy[0]['delivery_boy_id'] != 0) {

                $deli_boy_id = $res_boy[0]['delivery_boy_id'];
                $res_bonus = $function->get_data('delivery_boys', $columns = ['bonus,name,bonus_method'], 'id= ' . $deli_boy_id);
                ...
                if (strtolower($res_boy[0]['payment_method']) == 'cod') {
                    ...
                    $db->sql($sql);
                    $function->add_transaction($cash_received, $id, $res_boy[0]['delivery_boy_id'], 'delivery_boy_cash', 'Delivery boy collected COD');
                }
                ...
                if ($reward > 0) {
                    ...
                    $db->sql($sql);
                    $comission = $function->add_delivery_boy_commission($res_boy[0]['delivery_boy_id'], $reward, 'credit', 'Order Delivery Commission.');
                    ...
                    if ($res_count[0]['total'] == 0) {
                        if ($res_user[0]['friends_code'] != '') {
                            ...
                            if (!empty($friend_user))
                                $function->add_wallet_transaction($id, 'credit', floor($bonus_amount), $friend_user[0]['id'], 'Refer & Earn Bonus on first order by ' . ucwords($res_data[0]['name']));
        ...
        if ($postStatus == 'received') {
            ...
            /* get order data */
            $user_id1 = $function->get_data('orders', $columns = ['user_id', 'total', 'delivery_charge', 'discount', 'final_total', 'payment_method', 'address', 'otp'], 'id=' . $id);

            /* get user data */
            $user_email = $function->get_data('users', $columns = ['email', 'name'], 'id=' . $user_id1[0]['user_id']);
            $subject = "Order received successfully";
        ...
        if ($db->update('orders', 'id=' . $id, $final_status)) {
            ...
            if ($postStatus != 'received') {
                $user_data = $function->get_data('users', $columns = ['name', 'email', 'mobile', 'country_code'], 'id=' . $user_id);
    ...
    */
    ...
    foreach ($res as $row) {
        ...
        for ($j = 0; $j < count($res[0]['items']); $j++) {
            ...
            $res[$i]['items'][$j]['image'] = !empty($res[$i]['items'][$j]['image']) ? DOMAIN_URL . $res[$i]['items'][$j]['image'] : array();
            $res[$i]['items'][$j]['deliver_by'] = !empty($res[$i]['items'][$j]['deliver_by']) ? $res[$i]['items'][$j]['deliver_by'] : "";
            $res[$i]['items'][$j]['rate'] = !empty($res[$i]['items'][$j]['rate']) ? $res[$i]['items'][$j]['rate'] : "";
            $res[$i]['items'][$j]['review'] = !empty($res[$i]['items'][$j]['review']) ? $res[$i]['items'][$j]['review'] : "";
            $res[$i]['items'][$j]['manufacturer'] = !empty($res[$i]['items'][$j]['manufacturer']) ? $res[$i]['items'][$j]['manufacturer'] : "";
