...
if (isset($_POST['add_products_review']) && $_POST['add_products_review'] == 1) {
    ...
    */
    ...
    if ($db->sql($sql)) {
        ...
        $db->sql($sql1);
        $data = $db->getResult();
        $data[0]['images'] = isset($data[0]['images']) ? json_decode($data[0]['images'], 1) : '';
    ...
    */
    ...
    foreach ($res as $row) {
        ...
        if (!empty($res1)) {
            ...
            $row['images'] = isset($row['images']) ? json_decode($row['images'], 1) : [];
            ...
            $row['user_id'] = (!empty($row['user_id'])) ? $row['user_id'] : '';
