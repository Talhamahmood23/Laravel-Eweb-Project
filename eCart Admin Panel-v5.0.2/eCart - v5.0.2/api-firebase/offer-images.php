...
if ((isset($_POST['add-image'])) && ($_POST['add-image'] == 1)) {
    ...
    if (!empty($section_position)) {
        $section_data = $fn->get_data('sections', $columns = ['title'], 'id =' . $section_position);
    ...
    $offer_type = $db->escapeString($fn->xss_clean($_POST['offer_type']));
    $offer_image_url = $db->escapeString($fn->xss_clean($_POST['offer_image_url']));
    $id = ($offer_type != 'default') && ($offer_type != 'offer_image_url') ? $db->escapeString($fn->xss_clean($_POST[$offer_type])) : "0";
    ...
    if (empty($error['image'])) {
        ...
        $sql = "INSERT INTO `offers`(`image`,`position`,`section_position`,`type`,`offer_type`,`offer_type_id`,`offer_image_url`)
         VALUES ('$upload_image','$position','$section_name','$type','$offer_type','$id','$offer_image_url')";
        $db->sql($sql);
        $res = $db->getResult();
    ...
    if (!empty($result)) {
        ...
        foreach ($result as $row) {
            ...
            if ($row['offer_type'] == 'category') {
                ...
                $sql = 'select `name` from category where id = ' . $row['offer_type_id'] . ' order by id desc';
                $db->sql($sql);
            ...
            if ($row['offer_type'] == 'products') {
                ...
                $sql = 'select `name` from products where id = ' . $row['offer_type_id'] . ' order by id desc';
                $db->sql($sql);
                $result1 = $db->getResult();
            ...
            $temp['offer_type_id'] = $row['offer_type_id'];
            $temp['slug'] = isset($row['slug']) ? $row['slug'] : "";
            $temp['id'] = $row['id'];
