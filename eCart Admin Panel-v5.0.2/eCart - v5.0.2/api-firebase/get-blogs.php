...
if (isset($_POST['get_blogs']) && !empty($_POST['get_blogs']) && $_POST['get_blogs'] == 1) {
    ...
    if (!empty($res)) {
        ...
        $category_slug = $db->escapeString(trim($_POST['category_slug']));
        $sql = "SELECT name FROM blog_categories WHERE slug = '$category_slug'";
        $db->sql($sql);
        $category = $db->getResult();
        $category_name = (!empty($category)) ? $category[0]['name'] : '';

        $response['error'] = true;
        $response['message'] = ($category_name !== '') ? "Blog not found in category $category_name" : 'Data not Found!';
        $response['category_name'] = ($category_name) ? $category_name : 'Data not Found!';
