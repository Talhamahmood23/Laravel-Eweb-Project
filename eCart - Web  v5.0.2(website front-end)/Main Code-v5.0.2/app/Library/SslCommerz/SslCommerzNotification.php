...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    public function __construct()
    {
        ...
        $this->setStoreId(Cache::get('payment_methods')->ssl_store_id);
        $this->setStorePassword(Cache::get('payment_methods')->ssl_store_password);
    }
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if ($merchant_trans_id != "" && $merchant_trans_amount != 0) {
            ...
            if ($this->SSLCOMMERZ_hash_verify($post_data, $this->getStorePassword())) {
                ...
                if(Cache::get('payment_methods')->ssl_mode =="sandbox"){
                ...
                elseif(Cache::get('payment_methods')->ssl_mode =="production"){
                    $requested_url = ($this->config['apiDomainProduction'] . $this->config['apiUrl']['order_validate'] . "?val_id=" . $val_id . "&store_id=" . $store_id . "&store_passwd=" . $store_passwd . "&v=1&format=json");
                }
...
//                    }
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        elseif(Cache::get('payment_methods')->ssl_mode =="production"){
            $this->setApiUrl($this->config['apiDomainProduction'] . $this->config['apiUrl']['make_payment']);
        }
