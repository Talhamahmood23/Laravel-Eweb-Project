...
<section class="section-content footerfix padding-bottom fullscreen__shopcart">
    <a href="#" id="scroll"><span></span></a>

    <div class="container mt-5">
        <div class="outer_box px-2 py-4 px-md-4 py-md-3 bg-light shadow-sm rounded">
                            ...
                            <div class="table-responsive">
                                <table id="myTable" class="table ">
                                    ...
                                    <tbody id="movecart">
                                        ...
                                        @endphp
                                        <tr class="cart1price" id="{{ $p->product_variant_id ?? $p->id }}">
                                            <td>
                                                <a href="{{ route('product-single', $p->slug ?? $p->item->slug) }}">
                                            ...
                                            <td class="cart">
                                                ...
                                                <form action="{{ route('cart-update', $p->product_id) }}" method="POST" class="cartEdit cartEditpage">
                                                    ...
                                                    <input type="hidden" name="child_id" value="{{ $p->product_variant_id ?? $p->id }}">
                                            ...
                                            <td>
                                                <div class="price-wrap">
                                                    ...
                                                    <small class="text-muted">
                                                        ...
                                                        @endphp
                                                        {{ get_price($tax_price) }}{{ ($qty > 0) ? ' each' : '' }}

                                                        @else
                                                        {{ get_price($p->discounted_price) }}{{ ($qty > 0) ? ' each' : '' }}</small>
                                                    @endif
                                                    @endif
                                                    @endif
                                                </div>
                                            </td>
                                            <td class="text-end checktrash">
                                                ...
                                                <button class="btn btn-light btn-round cartDeletepage" data-varient="{{ $p->product_variant_id ?? $p->id }}"><em class="fas fa-trash-alt"></em></button>
                                                @if(isLoggedIn())<p class="save_for_later" data-varient="{{ $p->product_variant_id ?? $p->id }}"> Save for Later </p>@endif
                                            </td>
                                        </tr>
                                        @endif
                                        @endforeach
        ...
        <div class="outer_box px-2 py-4 px-md-4 py-md-3 bg-light shadow-sm rounded">
            <div class="row justify-content-center">
                <main class="col-md-12 mt-3">
                    <div class="card">
                        <div class="outer_box px-2 py-4 px-md-4 py-md-3 shadow-sm rounded">
                                            ...
                                            <td class="cart">
                                                ...
                                                <form action="{{ route('cart-update', $p->product_id) }}" method="POST" class="cartEdit cartEditpage">
                                                    ...
                                                    <div class="button-container col">
                                                        ...
                                                        <input class="form-control qtyPicker" type="number" name="qty" data-min="1" min="1" max="{{ intval(getMaxQty($p->item[0])) }}" data-max="{{ intval(getMaxQty($p->item[0])) }}" data-max-allowed="{{ Cache::get('max_cart_items_count') }}" value="{{ $p->qty }}" readonly>
