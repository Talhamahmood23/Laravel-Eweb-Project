...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static">
   <div class="login_model modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <button type="button" class="close close-btn theme1" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <!-- Nav tabs -->
                            ...
                            <div class="card mx-auto">
                                <div class="card-body loginsec modal-body">
                                    ...
                                    <form method="POST" action="{{route('login')}}">
                                        ...
                                        <div class="form-group">
                                            <a href="#" class="float-right forgot_pass" id='btnForgot'>{{__('msg.forgot_password')}}</a>
                                            <button type="submit" class="btn btn-primary "> {{__('msg.login')}}  </button>
                                        </div>
                                    </form>
                                    <p class="text-center mt-4">{{__('msg.don')}}'{{__('msg.t_have_account')}} <a href="#" id="btnRegister" class="btn btn-primary">{{__('msg.sign_up')}}</a></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="{{ (isset($data['code']) && $data['code'] != '') ? '' : 'error-hide' }}" id="cardRegister">
                            <div class="card mx-auto">
                                <div class="card-body modal-body">
                                    ...
                                    <p class="text-center mt-4 alreadyLogin">{{__('msg.already_have_account')}}<a href="#" class="btn btn-primary btnLogin ml-1">{{__('msg.login')}} </a></p>
                                    <p class="text-center mt-4 backToLogin error-hide" id="backToLogin">{{__('msg.back_to')}} <a href="#" class="btn btn-primary btnLogin ml-1">{{__('msg.login')}} </a></p>
                                </div>
                            </div>
                        </div>
