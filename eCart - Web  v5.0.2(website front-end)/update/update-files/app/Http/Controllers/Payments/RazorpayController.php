        ...
        try{
            ...
            if($this->isFromWallet == false){
                ...
                if ($response['success'] && intval($response['data']['order_id'])){
                    ...
                    $trans = $this->add_transaction($response['data']['order_id'], "razorpay", $request->razorpay_payment_id??'', true, msg('order_success') , $amount);
                    ...
                    $this->add_transaction($response['data']['order_id'], "razorpay", $request->razorpay_payment_id??'', true, msg('order_success') , $amount);

                    $error = $response['message']??msg('order_error');
                }
            }else{
                $response = $this->topup_wallet($amount, "razorpay");
