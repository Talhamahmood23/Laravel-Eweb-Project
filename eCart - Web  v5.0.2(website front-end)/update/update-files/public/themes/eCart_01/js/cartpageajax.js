            ...
            success: function (response) {
                ...
                if (response.subtotal > 0) {
                    ...
                    $.each(response.items, function (i, e) {
                        ...
                        html += '<tr class="cart1price cartpage">' +
                                '<td>' +
                                '<a href="/product/' + e.slug + '">' +
                                '<figure class="itemside">' +
                                '<div class="aside">' +
                                '<img src="' + e.image + '" class="img-sm" alt="' + e.name + '">' +
                                '</div>' +
                                '<figcaption class="info">' +
                                '<a href="/product/' + e.slug + '" class="title text-dark">' + e.name + '</a>' +
                                ...
                                '<input class="form-control qtyPicker" type="number" name="qty" data-min="1" min="1" max="' + e.stock + '" data-max="' + e.stock + '" data-max-allowed="' + response.max_cart_items_count + '" value="' + qty + '" readonly>' +
                        ...
                        if (e.discounted_price > 0) {
                            if (e.tax_percentage > 0) {
                                ...
                                html +=
                                        response.currency +
                                        parseFloat(tax_price * qty).toFixed(2);
                            } else {
                                html +=
                                        response.currency +
                                        parseFloat(
                                                e.discounted_price * qty
                                                ).toFixed(2);
                            }
                        } else {
                            if (e.tax_percentage > 0) {
                                ...
                                html +=
                                        response.currency +
                                        parseFloat(e.price * qty).toFixed(2);
                            }
                        }
                        html += '</var><br/>';
                        if (qty > 1) {
                        ...
                        html += '</div>' +
                                ...
                                '<button class="btn btn-light btn-round cartDeletepage mx-1" data-varient="' + e.product_variant_id + '"><em class="fas fa-trash-alt"></em></button>';
                        if (user_id) {
                    ...
                    if (response.subtotal > 0) {
                        ...
                        if (user_id == null) {
                            ...
                            if (response.subtotal >= response.min_order_amount) {
                                html += '<a id="door_step_delivery" href="'+home+'checkout/addresse" class="btn btn-primary">' + response.checkout_msg + ' <em class="fa fa-arrow-right"></em></a>  <a id="pickup_from_store" href="'+home+'checkout" class="btn btn-primary">' + response.checkout_msg + ' <em class="fa fa-arrow-right"></em></a>';
                            } else {
            ...
            if (response.subtotal > 0) {
                ...
                $.each(response.items, function (i, e) {
                    ...
                    var product_id = e.product_id;
                        ...
                        var maxqty = 0;
                        $.each(response.items, function (i, e) {
                            if (e.qty > 0 && (e) && e.unit === 'gm' && e.product_id === product_id) {
                                var stock_smallvarient = e.qty * e.measurement;
                                cart_stock_cart_page += stock_smallvarient;
                                maxqty = e.stock;
                            }
                            if (e.qty > 0 && (e) && e.unit === 'kg' && e.product_id === product_id) {
                    ...
                    html += '<tr class="cart1price cartpage">' +
                            ...
                            '<td class="cart">' +
                            '<div class="price-wrap cartShow">' + qty + '</div>' +
                            '<form action="' + home + 'cart/update/' + e.product_id + '" method="POST" class="cartEdit cartEditpage">' +
                    ...
                    html += '</div>' +
                            '</td>' +
                            '<td class="text-right checktrash">' +
                            '<button class="btn btn-light btn-round btnEdit cartShow mx-1"> <em class="fa fa-pencil-alt"></em></button>' +
                            '<button class="btn btn-light btn-round cartSave cartEdit cartEditpage mx-1"> <em class="fas fa-check"></em></button>' +
                            '<button class="btn btn-light btn-round btnEdit cartEdit mx-1"> <em class="fa fa-times"></em></button>' +
                            '<button class="btn btn-light btn-round cartDeletepage mx-1" data-varient="' + e.product_variant_id + '"><em class="fas fa-trash-alt"></em></button>';
                    if (user_id) {
                        html += '<br/>  <p class="save_for_later" data-varient="' + e.product_variant_id + '"> Save for Later </p>';
                    }
...
$(function () {
    $(document).on('click', '.save_for_later', function (e) {
        ...
        var varient = $(this).data('varient');
        $.ajax({
            url: home + "save-for-later/" + varient,
            type: 'POST',
            dataType: 'json',
            success: function (response) {
                ...
                var savelater = '<tr class="cart1price cartpage">' +
                        '<td>' +
                        ...
                        '<form action="' + home + 'cart/update/' + response.product[0].item[0].product_id + '" method="POST" class="cartEdit cartEditpage">' +
                        //@csrf
                ...
                savelater += '</div>' +
                        '</td>' +
                        '<td class="text-right checktrash">' +
                        '<p class="move_to_cart" data-varient="' + response.varient_id + '"> Move to Cart </p>' +
                        '</td>' +
                        '</tr>';
                $("#savelater").append(savelater);
                $("#price_section").empty();
                $("#empty_savelater").empty();
    ...
    $(document).on('click', '.move_to_cart', function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                var movecart = '<tr class="cart1price cartpage">' +
                        '<td>' +
                        '<a href="' + home + 'product/' + response.product[0].item[0].slug + '">' +
                        '<figure class="itemside">' +
                        '<div class="aside">' +
                ...
                movecart += '</div>' +
                        ...
                        '<button class="btn btn-light btn-round cartSave cartEdit cartEditpage mx-1"> <em class="fas fa-check"></em></button>' +
                        '<button class="btn btn-light btn-round btnEdit cartEdit mx-1"> <em class="fa fa-times"></em></button>' +
                        '<button class="btn btn-light btn-round cartDeletepage mx-1" data-varient="' + response.product[0].item[0].product_variant_id + '"><em class="fas fa-trash-alt"></em></button>' +
                        '<br/>  <p class="save_for_later" data-varient="' + response.varient_id + '"> Save for Later </p>' +
                        '</td>' +
                        '</tr>';
